...
$(document).ready(function () {
    ...
    $('#estudiante').change(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                //console.log(data.disciplinarios.length);
                ...
                var out='';
                for (i = 0; i < data.disciplinarios.length; i++) {
                    out += '<tr>';
                    out += '<td>';
                    out += data.disciplinarios[i].dis_fecha;
                    out += '</td>';
                    out += '<td>';
                    out += data.disciplinarios[i].ptd_descripcion;
                    out += '</td>';
                    out += '</tr>';
                }
                $('#tabla-indisciplina').html(out);
            },
        });
        return false;
    });
